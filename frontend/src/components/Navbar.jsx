  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 z-50 w-full border-b transition-colors duration-500 ${
        scrolled
          ? "border-[#2563FF]/15 bg-[#050505]/70 backdrop-blur-xl"
      ...
      <nav className="mx-auto flex h-20 max-w-[1600px] items-center justify-between px-6 md:px-12 lg:px-20">
        ...
        <div className="hidden items-center gap-10 md:flex">
              ...
              return (
                <li key={link.href}>
                  <button
                    ...
                    className={`group relative text-sm tracking-wide transition-colors duration-300 hover:text-[#F5F7FA] ${
                      isActive ? "text-[#F5F7FA]" : "text-[#94A3B8]"
                    }`}
                  >
                    {link.label}
                    <span
                      className={`absolute -bottom-1 left-0 h-px bg-[#2563FF] transition-all duration-300 ${
          ...
          <button
            ...
            data-cursor="hover"
            onClick={() => go("#contact")}
            className="group flex items-center gap-1.5 border border-[#2563FF]/40 px-5 py-2.5 text-sm text-[#F5F7FA] transition-all duration-300 hover:border-[#2563FF] hover:bg-[#2563FF] hover:shadow-glow"
          >
            Let's Talk
